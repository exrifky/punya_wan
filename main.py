...
option = st.sidebar.selectbox(
    'Daftar Menu',
    ('Home', 'Prediksi Tunggal', 'Prediksi Berkelompok')
)

if option == 'Home' or option == '':
    st.write("""## Tentang Aplikasi""") #menampilkan halaman utama
    st.write("Aplikasi ini merupakan aplikasi yang digunakan untuk keperluan memprediksi masa studi "
             "Mahasiswa Magister Ilmu Komputer, Fakultas Ilmu Komputer Universitas Brawijaya.")
    st.write("Proses prediksi dilakukan dengan menggunakan data seleksi masuk calon mahasiswa.")
    st.write("""### Fitur Aplikasi""")
    st.write("1. Melakukan prediksi tunggal")
    st.write("2. Melakukan prediksi berkelompok")

    st.write('Semoga membantu dan dapat bermanfaat :male-student: :female-student:')

elif option == 'Prediksi Tunggal':
    st.write("""## Prediksi Tunggal Mahasiswa""")  # menampilkan judul halaman dataframe
...
elif option == 'Prediksi Berkelompok':
    st.write("""## Prediksi Berkelompok Mahasiswa""")  # menampilkan judul halaman dataframe
    ...
    st.write("**NOTE:** Rubah data yang mengandung koman (,) dengan titik (.).")
    st.write("Misalnya 77,4 menjadi 77.4")


    if st.button('Paste Data'):
        df3 = pd.read_table("test1.txt", delimiter="	")
        df3.rename(columns={'1': "No", '2': "No. Pendaftaran", '3': 'Nama Lengkap', '4': 'Asal PT',
        ...
        df3.replace(to_replace="Teknologi Media Game dan Piranti Bergerak", value="1", inplace=True)
        df3.replace(to_replace="Jaringan Berbasis Informasi", value="2", inplace=True)
        df3.replace(to_replace="Rakayasa Perangkat Lunak", value="3", inplace=True)
        df3.replace(to_replace="Sistem Cerdas / Computer vision", value="4", inplace=True)
        df3.replace(to_replace="Sistem Informasi", value="5", inplace=True)

        df3.replace(to_replace="Tidak ada", value="1", inplace=True)
        df3.replace(to_replace="Preparation", value="2", inplace=True)
        df3.replace(to_replace="iBT", value="3", inplace=True)
        df3.replace(to_replace="ITP", value="4", inplace=True)

        df3.replace(to_replace="Tidak ada", value="1", inplace=True)
        df3.replace(to_replace="Lokal", value="2", inplace=True)
        df3.replace(to_replace="OTO Bappenas", value="3", inplace=True)

        df3.replace(to_replace="C / Baik", value="1", inplace=True)
